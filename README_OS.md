	...
	- [ Benchmark安装说明](#head3)
		...
		- [ 一、Benchmark环境配置、安装要求](#head5)
			...
			- [ 2.数据集制作](#head12)
				- [ 数据集下载](#head13)
				- [ TFReord制作](#head14)
		- [ 二、Benchmark测试规范](#head15)
			- [ 配置运行参数](#head16)
			- [ 运行benchmark](#head17)
			- [ 停止实验](#head18)
		- [ 三、测试参数设置及推荐环境配置](#head19)
			- [ 可变设置](#head20)
			- [ 推荐环境配置](#head21)
...
### <span id="head4"> 本文用于在物理机环境下运行Benchmark</span>
...
#### <span id="head6"> 1.基础环境配置</span>

##### <span id="head7"> 配置共享文件系统</span>
...
```
mount NFS服务器IP:/userhome /userhome
```
...
##### <span id="head9"> 所有节点安装slurm</span>
...
*密钥路径为/etc/munge/munge.key，master节点使用scp将master节点上的munge.key拷贝到所有节点的相同路径下*

```
scp -P ssh端口 /etc/munge/munge.key root@SLAVE_IP:/etc/munge/
```
...
如果STATE列的状态后面带*则该节点网络出现问题，master无法访问该节点。

##### <span id="head10"> 目录调整</span>
...
#### <span id="head12"> 2.数据集制作</span>

##### <span id="head13"> 数据集下载</span>
...
### <span id="head15"> 二、Benchmark测试规范</span>
...
| 15   |        --warmup_3        |    warm up机制第三轮epoch数     |       80        |
...
#### <span id="head17"> 运行benchmark</span>
...
### <span id="head19"> 三、测试参数设置及推荐环境配置</span>

#### <span id="head20"> 可变设置</span>
